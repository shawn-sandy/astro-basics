...
- [ ] 1.1 Remove non-unique index `idx_users_email` from PART 3: INDEXES section (line ~103)
- [ ] 1.2 Add DO block for partial unique index `idx_users_email_unique` in PART 3: INDEXES section
- [ ] 1.3 Include IF NOT EXISTS check in DO block for idempotent constraint creation
- [ ] 1.4 Add RAISE NOTICE messages for constraint creation success/skip
- [ ] 1.5 Add comment explaining partial unique index design (allows NULL, prevents duplicate non-NULL)
- [ ] 1.6 Add COMMENT ON INDEX statement in PART 4: DOCUMENTATION section
- [ ] 1.7 Update verification section (PART 5) to check idx_users_email_unique existence
- [ ] 1.8 Update success message to report "7 indexes for performance (including email uniqueness)"
...
- [ ] 2.1 Open `scripts/migrations/rollback_001_core_schema.sql`
- [ ] 2.2 Add `DROP INDEX IF EXISTS idx_users_email_unique;` in indexes section
- [ ] 2.3 Ensure proper ordering (drop indexes before dropping tables)
...
- [ ] 3.1 Open `scripts/migrations/004_clerk_email_verification.sql`
- [ ] 3.2 Update header comment block with deprecation notice
- [ ] 3.3 Add note: "For new installations, use migration 001 which includes this constraint"
- [ ] 3.4 Add note: "This migration is safe to apply on existing databases (idempotent design)"
- [ ] 3.5 Keep all existing validation and constraint logic intact
- [ ] 3.6 Ensure migration 004 still works independently for existing deployments

## 4. Documentation Updates

- [ ] 4.1 Update `CLAUDE.md` Supabase Migrations section
- [ ] 4.2 Document new 2-migration path for fresh installations (001 + 002)
- [ ] 4.3 Document that migration 004 is for existing deployments only
- [ ] 4.4 Add note about idempotent design ensuring safety for all installation paths
- [ ] 4.5 Update migration file list to reflect consolidation
- [ ] 4.6 Update "Applying Migrations" section with new recommended order
- [ ] 4.7 Check if `project-docs/05-database/supabase-migration-refactor-plan.md` needs updates
- [ ] 4.8 Update migration refactor plan if it exists to reflect consolidation
