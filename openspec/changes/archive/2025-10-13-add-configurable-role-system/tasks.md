...
- [x] 1.1.1 Create `config/roles.config.ts` with TypeScript configuration structure
- [x] 1.1.2 Define role configuration interface (`RoleDefinition`, `RoleConfig`)
- [x] 1.1.3 Create default configuration with 3-tier system (member, admin, super_admin)
- [x] 1.1.4 Add JSDoc comments explaining each configuration field
- [x] 1.1.5 Export configuration as `const` assertion for type inference

### 1.2 Implement Configuration Validation

- [x] 1.2.1 Create `scripts/lib/role-validator.ts`
- [x] 1.2.2 Install Zod dependency (`npm install zod`)
- [x] 1.2.3 Define Zod schema for role name validation (lowercase alphanumeric, underscores only)
- [x] 1.2.4 Define Zod schema for hierarchy level validation (1-10 range)
- [x] 1.2.5 Define Zod schema for role label validation (1-50 characters)
- [x] 1.2.6 Implement core role requirement validation (member, admin, super_admin mandatory)
- [x] 1.2.7 Implement unique role name validation
- [x] 1.2.8 Implement unique hierarchy level validation
- [x] 1.2.9 Create blocklist for SQL keywords (user, role, select, etc.)
- [x] 1.2.10 Create blocklist for TypeScript keywords (class, interface, type, etc.)
- [x] 1.2.11 Implement custom validation error messages
- [x] 1.2.12 Export `validateRoleConfig(config)` function
- [x] 1.2.13 Write unit tests for validation scenarios (valid and invalid configs)
...
- [x] 2.1.1 Create `scripts/setup-roles.ts` main script file
- [x] 2.1.2 Install inquirer dependency (`npm install inquirer` and `@types/inquirer`)
- [x] 2.1.3 Implement CLI header/banner with project name and version
- [x] 2.1.4 Implement configuration file detection (check if `config/roles.config.ts` exists)
- [x] 2.1.5 Implement "first run" vs "update" workflow detection
- [x] 2.1.6 Add colorized console output using chalk or native Node.js colors
...
- [x] 3.1.1 Create `scripts/lib/role-generator.ts`
- [x] 3.1.2 Implement `generateRoleTypes(config)` function
- [x] 3.1.3 Generate `UserRole` union type from config role names
- [x] 3.1.4 Generate `USER_ROLES` const array from config role names
- [x] 3.1.5 Generate `ROLE_HIERARCHY` const object from config levels
- [x] 3.1.6 Generate `ROLE_LABELS` const object from config labels
- [x] 3.1.7 Add TypeScript `as const` assertions for type narrowing
- [x] 3.1.8 Add file header warning ("AUTO-GENERATED - DO NOT EDIT MANUALLY")
- [x] 3.1.9 Add source reference comment ("Generated from: config/roles.config.ts")
- [x] 3.1.10 Add generation timestamp in comment
- [x] 3.1.11 Add command to regenerate ("Run: npm run setup:roles")

### 3.2 Implement Type Writer

- [x] 3.2.1 Create function to write generated types to `src/types/generated-roles.ts`
- [x] 3.2.2 Ensure output directory exists (`src/types/`) before writing
...
- [x] 3.2.5 Create backup of existing generated file before overwrite
- [x] 3.2.6 Write success message with file path
...
- [x] 3.3.3 Add comment explaining generated vs hardcoded separation
- [x] 3.3.4 Update JSDoc comments to reference generation process
...
- [x] 4.1.1 Create `scripts/lib/migration-generator.ts`
- [x] 4.1.2 Implement `generateRoleMigration(config)` function
- [x] 4.1.3 Generate PostgreSQL ENUM creation SQL with role names
- [x] 4.1.4 Add role descriptions as SQL comments (from labels)
- [x] 4.1.5 Generate idempotent SQL using `CREATE TYPE IF NOT EXISTS` pattern (or DO block)
- [x] 4.1.6 Generate transaction wrapper (BEGIN/COMMIT)
- [x] 4.1.7 Generate verification queries to confirm ENUM created
- [x] 4.1.8 Add migration file header with description and timestamp

### 4.2 Implement Migration Numbering

- [x] 4.2.1 Create function to scan `scripts/migrations/` for existing migrations
- [x] 4.2.2 Implement auto-increment logic for migration number (find max + 1)
- [x] 4.2.3 Format migration number with zero-padding (001, 002, etc.)
...
- [x] 4.3.1 Create `generateRollbackMigration(config)` function
- [x] 4.3.2 Generate SQL to drop ENUM type (with CASCADE if needed)
- [x] 4.3.3 Add safety warnings in rollback migration comments
- [x] 4.3.4 Generate idempotent rollback using `DROP TYPE IF EXISTS`
- [x] 4.3.5 Write rollback migration to `scripts/migrations/rollback_XXX_user_roles.sql`

### 4.4 Implement Migration Writer

- [x] 4.4.1 Create function to write migration SQL to file
- [x] 4.4.2 Use naming pattern: `XXX_user_roles.sql` (where XXX is number)
- [x] 4.4.3 Write both forward and rollback migrations
- [ ] 4.4.4 Validate SQL syntax (basic parsing check)
- [x] 4.4.5 Write success message with migration file paths

### 4.5 Turso Migration Support (Optional)

- [x] 4.5.1 Generate Turso-compatible migration (TEXT column with CHECK constraint)
- [x] 4.5.2 Detect database provider from environment (Supabase vs Turso)
- [x] 4.5.3 Generate appropriate migration for detected provider
...
- [x] 5.2.1 Add `"setup:roles": "tsx scripts/setup-roles.ts"` script
- [x] 5.2.2 Add `"setup:roles:dry-run": "tsx scripts/setup-roles.ts --dry-run"` script
- [x] 5.2.3 Add `"validate:roles": "tsx scripts/lib/role-validator.ts"` script (standalone validation)
...
- [x] 6.1.1 Create `tests/scripts/role-validator.test.ts`
- [x] 6.1.2 Test valid configuration passes validation
- [x] 6.1.3 Test invalid role name rejected (uppercase, special chars, etc.)
- [x] 6.1.4 Test invalid hierarchy level rejected (out of range, non-integer)
- [x] 6.1.5 Test missing core roles rejected (member, admin, super_admin required)
- [x] 6.1.6 Test duplicate role names rejected
- [x] 6.1.7 Test duplicate hierarchy levels rejected
- [x] 6.1.8 Test SQL keyword blocklist enforced
- [x] 6.1.9 Test TypeScript keyword blocklist enforced
- [x] 6.1.10 Test custom error messages are descriptive

### 6.2 Unit Tests for Type Generation

- [x] 6.2.1 Create `tests/scripts/role-generator.test.ts`
- [x] 6.2.2 Test UserRole union type generated correctly
- [x] 6.2.3 Test USER_ROLES array generated correctly
- [x] 6.2.4 Test ROLE_HIERARCHY object generated correctly
- [x] 6.2.5 Test ROLE_LABELS object generated correctly
- [x] 6.2.6 Test `as const` assertions present in generated code
- [x] 6.2.7 Test file header comments generated correctly
- [x] 6.2.8 Test generated TypeScript is syntactically valid
- [x] 6.2.9 Test generated types are importable and usable

### 6.3 Unit Tests for Migration Generation

- [x] 6.3.1 Create `tests/scripts/migration-generator.test.ts`
- [x] 6.3.2 Test PostgreSQL ENUM SQL generated correctly
- [x] 6.3.3 Test migration number auto-increment works correctly
- [ ] 6.3.4 Test migration numbering detects conflicts
- [x] 6.3.5 Test rollback migration generated correctly
- [x] 6.3.6 Test idempotency patterns (IF NOT EXISTS) present
- [x] 6.3.7 Test transaction wrappers (BEGIN/COMMIT) present
- [x] 6.3.8 Test migration file naming follows pattern
...
- [x] 7.1.1 Create `docs/guide/configurable-roles.md`
- [x] 7.1.2 Document "Getting Started" with setup wizard
- [x] 7.1.3 Document configuration file structure and options
- [x] 7.1.4 Document how to add custom roles
- [x] 7.1.5 Document how to update existing roles
- [x] 7.1.6 Document migration workflow after setup
- [x] 7.1.7 Add troubleshooting section for common errors
- [x] 7.1.8 Add examples for different use cases (blog, SaaS, forum)
...
- [x] 7.4.1 Update `CLAUDE.md` with configurable role system section
