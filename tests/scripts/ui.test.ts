...
describe('UI Module', () => {
  ...
  describe('getDataAttributes', () => {
    ...
    it('should filter by prefix correctly', () => {
      ...
      // Simulate prefix filtering with improved logic
      const data: Record<string, any> = {}
      const prefix = 'form'
    ...
    it('should handle camelCase keys correctly', () => {
      const element = {
        dataset: {
          formMySpecialProperty: 'special',
          formXMLHttpRequest: 'xhr',
          formDataForm: 'nested',
        },
      }

      // Simulate camelCase handling
      const data: Record<string, any> = {}
      const prefix = 'form'
      ...
      expect(data.mySpecialProperty).toBe('special')
      expect(data.xMLHttpRequest).toBe('xhr')
      expect(data.dataForm).toBe('nested') // Should preserve the second "Form"
    })

    it('should handle keys without prefix unchanged', () => {
      const element = {
        dataset: {
          someProperty: 'value1',
      ...
      expect(data.someProperty).toBe('value1')
      expect(data.AnotherProperty).toBe('value2')
    })
  })
