...
- `src/pages/index.astro` (modified) — render `HomeHero` into the layout's `header` slot
...
6. Create `src/components/astro/HomeHero.astro` with a mono eyebrow, a left-aligned display headline, a deck at roughly a third of the headline size, two calls to action and a specimen figure pairing a rendered `Card` with the import line that produces it, then render it from `src/pages/index.astro` into the `header` slot that `src/layouts/Base.astro:102` already branches on Why: the shared `Header.astro` serves every route, so using the layout's existing slot escape hatch confines the layout change to the homepage without adding a conditional to a shared component Verify: the homepage display-to-deck font-size ratio is at or above 3.0 with `text-align: left`, and `/about` still renders the original `Header`.
