import { executeTransaction, isTursoConfigured, type MessageData } from '../libs/turso'
...
// Message templates
const messages: Array<MessageData & { daysAgo: number; is_read?: boolean; is_archived?: boolean }> =
  [
    {
      daysAgo: 0,
      name: 'Sarah Johnson',
...
async function seedMessages() {
  if (!isTursoConfigured()) {
    console.error(
      '❌ Turso database is not configured. Please set TURSO_DATABASE_URL and TURSO_AUTH_TOKEN.'
    )
    process.exit(1)
  }
