...
<CardGrid>
  ...
  <Card title="Supabase Database" icon="database">
    PostgreSQL storage through Supabase, accessed via a unified `getDatabase()` abstraction layer
    with Row Level Security policies and an interactive setup wizard.
  </Card>
...
- **🔄 Database Abstraction**: Supabase (PostgreSQL) behind a unified `getDatabase()` API
...
- **Supabase**: PostgreSQL database accessed through a unified abstraction layer
