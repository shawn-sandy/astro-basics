...
<Aside type="caution" title="Depends on 001_core_schema.sql">
  ...
  table up first, owner-readable only since the rows hold personal data:
  `install -m 600 /dev/null messages-backup.sql && pg_dump "$DATABASE_URL" --table=messages > messages-backup.sql`.
</Aside>
