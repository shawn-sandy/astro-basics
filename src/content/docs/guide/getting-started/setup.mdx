...
- ✅ **Database Support** - Supabase (PostgreSQL) backend
...
The project uses **Supabase** (PostgreSQL) for real-time features, native Clerk integration, and RLS.
...
- Prompt for your Supabase project URL, anon key, and service role key (required for database queries)
- Test the connection
...
Migrations are SQL files in `scripts/migrations/`. Apply each one with `psql`, or paste it into the Supabase SQL editor:

```bash
# Export your Supabase connection string first (Project Settings → Database).
# It is not the API URL above: psql needs the PostgreSQL connection string.
...
# Apply the contact messages table, required by the contact form and /dashboard/messages
...
# Data management
npm run db:seed:messages    # Seed the messages table
```
...
# "relation \"messages\" does not exist" means this one has not been applied
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -f scripts/migrations/006_messages.sql
...
1. **Explore Features**
   ...
   - [Environment Configuration](/guide/environment-configuration/) - Database and service settings
