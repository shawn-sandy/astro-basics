...
There is no client-side Supabase helper. Query Supabase on the server — in an
Astro page or an API route, with the helpers above — and pass the result to the
React component as props, or have the component fetch it from an API route such
as `/api/user/profile`.
