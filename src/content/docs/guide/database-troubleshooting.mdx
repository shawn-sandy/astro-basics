...
<Tabs>
  ...
  <TabItem label="Manual Fix">
    <Steps>
      1. Check which required variables are set (shows names and status, never the values):
         ```bash
         npm run db:status
         ```
...
<Tabs>
  <TabItem label="Quick Schema Check">
    ```bash
    npm run db:schema
    # Reads the messages table and checks the columns the app needs.
    # Exits non-zero when the read fails or a column is missing.
    ```
  </TabItem>
...
The contact form returns a 500, `/dashboard/messages` shows nothing, and `npm run db:schema`
and `npm run db:manage test` both fail on the read. A fresh Supabase project has no
`messages` table.
...
See [Messages Table](/guide/messages-table/) for the schema this creates and its RLS policy.
  ...
  <TabItem label="Configuration Debugging">
    <Steps>
      1. **Compare Environment Files**:
         ```bash
         # Local: key names only, so no secret reaches the terminal
         grep -o '^SUPABASE_[A-Z_]*=' .env | sort
...
1. **Fix File Permissions**:
   ```bash
   chmod 600 .env  # owner-only: it holds SUPABASE_SERVICE_ROLE_KEY
   chmod +x scripts/*.js
   ```
