...
<Aside type="note" title="Supabase is the only database">
  Application code should not build a client from these credentials. Use the helpers in
  `#libs/supabase-native` (`getSupabaseServiceRole()`, `createServerSupabaseClient(token)`), which
  read this configuration and return `null` when Supabase is not configured.
</Aside>
