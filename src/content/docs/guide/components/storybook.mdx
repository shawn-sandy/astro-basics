...
<Aside type="caution" title="Alerts must declare data-visible">
  `@fpkit/acss` v6 ships `[role="alert"]:not([data-visible="true"]) { opacity: 0 }` so its
  own dismissible `<Alert>` can fade in. That selector outranks this project's `.alert`
  styles on specificity, so **any `role="alert"` element without `data-visible="true"`
  renders fully transparent** — visible to screen readers, invisible to everyone else.

Storybook caught this as a live bug in four components; all four now set the attribute,
and `tests/components/alert-visibility.test.tsx` scans the component sources so a new
alert cannot reintroduce it. If you add an alert, set `data-visible="true"` — or use
acss's own `<Alert>` component if you actually want the fade-in.
