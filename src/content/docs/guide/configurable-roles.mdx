...
- Automatically generate PostgreSQL (Supabase) database migrations
...
Replace `XXX` with the migration number `setup:roles` printed. `DATABASE_URL` is your Supabase connection string (Project Settings > Database). You can also paste the migration file into the Supabase SQL editor.
...
3. Apply the generated migration with `psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -f scripts/migrations/XXX_user_roles.sql`
