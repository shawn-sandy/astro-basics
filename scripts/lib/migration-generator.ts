 ...
 * Migration Generator
 *
 * Generates Supabase (PostgreSQL) migrations for role ENUM creation
...
/**
 * Detects whether the Supabase database is configured in the environment
 *
 * @returns 'supabase' when credentials are present, otherwise 'unknown'
 */
export function detectDatabaseProvider(): 'supabase' | 'unknown' {
  const hasSupabase = !!(process.env.SUPABASE_URL && process.env.SUPABASE_SERVICE_ROLE_KEY)

  return hasSupabase ? 'supabase' : 'unknown'
}
